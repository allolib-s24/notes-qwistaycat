...
vector<string> mappingType{"amp", "freq", "ampStri", "ampLow", "ampUp", "channel"};
...
public:
  ...
  virtual void onProcess(AudioIOData& io) override {
    ...
    if(hueMapping.getCurrent() == mappingType[0]) {
      ...
      hue = ampUp;
    } else if(hueMapping.getCurrent() == mappingType[5]){
      if (amp > 0.036000){ //channel 0
        hue = 0.14286;
      } else if (amp < 0.031000){ //channel 1
        hue = 0.28571;
      } else if (amp < 0.032000){ //channel 2
        hue = 0.42857;
      } else if (amp < 0.033000){ //channel 3
        hue = 0.57143;
      } else if (amp < 0.034000){ //channel 4
        hue = 0.71429;
      } else if (amp < 0.035000){ //channel 5
        hue = 0.85714;
      } else { //channel 6
        hue = 1;
      }
    ...
    if(satMapping.getCurrent() == mappingType[0]) {
      ...
      sat = ampUp;
    } else if(satMapping.getCurrent() == mappingType[5]) {
      if (amp > 0.036000){ //channel 0
        sat = 0.14286;
      } else if (amp < 0.031000){ //channel 1
        sat = 0.28571;
    ...
    if(valMapping.getCurrent() == mappingType[0]) {
      ...
      val = ampUp;
    } else if(valMapping.getCurrent() == mappingType[5]) {
      if (amp > 0.036000){ //channel 0
        val = 0.14286;
      } else if (amp < 0.031000){ //channel 1
        val = 0.28571;
